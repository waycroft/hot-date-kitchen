...
test("should validate input parameters", async () => {
  expect(retry("not a function", [])).rejects.toThrow(
    "asyncFn must be a function",
  );
  expect(retry(() => {}, "not an array")).rejects.toThrow(
    "ErrorTypes must be an array",
  );
  expect(retry(() => {}, [], { jitter: 2 })).rejects.toThrow(
    "jitter must be between 0 and 1.0",
  );
  expect(retry(() => {}, [], { maxRetries: -1 })).rejects.toThrow(
    "maxRetries must be non-negative",
  );
  expect(retry(() => {}, [], { retryInterval: -1 })).rejects.toThrow(
    "retryInterval must be non-negative",
  );
});
